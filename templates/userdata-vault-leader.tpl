...
sudo mkdir -pm 0755 /etc/vault.d/tls
sudo mkdir -pm 0755 /etc/ssl/vault


cat > /etc/ssl/vault/vault.crt <<EOF
${cert}
EOF

sudo chmod 755 /etc/ssl/vault/vault.crt 

cat > /etc/ssl/vault/me.key <<EOF
${key}
EOF

sudo chmod 755 /etc/ssl/vault/me.key
...
# vault_leader adds some test data to demonstrate that the cluster is connected to
#   the same data.
sleep 5
